...
class SubtitleExtractionThread(QThread):
    ...
    def _extract_with_ocr(self):
        """使用OCR提取硬字幕"""
        try:
            self.progress.emit("初始化OCR引擎...")
            
            # 选择OCR提取器
            if self.ocr_settings.get('use_improved_ocr', False):
                # 使用改进的OCR提取器
                ocr_extractor = ImprovedOCRExtractor(
                    languages=self.ocr_settings['languages'],
                    use_gpu=self.ocr_settings.get('use_gpu', True),
                    engine=self.ocr_settings.get('engine', 'auto')
                )
            else:
                # 使用原版OCR提取器
                ocr_extractor = VideoOCRExtractor(
                    languages=self.ocr_settings['languages'],
                    use_gpu=self.ocr_settings.get('use_gpu', True)
                )
            ...
            if self.output_dir:
                output_path = os.path.join(self.output_dir, f"{video_name}_ocr.srt")
            else:
                import tempfile
                temp_dir = tempfile.mkdtemp()
                output_path = os.path.join(temp_dir, f"{video_name}_ocr.srt")
            
            self.progress.emit("开始OCR识别...")
            
            # 为改进的OCR提取器提供进度回调
            if hasattr(ocr_extractor, 'extract_subtitles_from_video') and self.ocr_settings.get('use_improved_ocr', False):
                # 改进版本支持进度回调
                def progress_callback(message):
                    self.progress.emit(message)
                
                subtitles = ocr_extractor.extract_subtitles_from_video(
                    self.video_path,
                    output_path,
                    sample_interval=self.ocr_settings['sample_interval'],
                    subtitle_region=self.ocr_settings['subtitle_region'],
                    confidence_threshold=self.ocr_settings['confidence_threshold'],
            ...
            self.progress.emit(f"OCR识别失败: {str(e)}")
            return []
...
class SubtitleWERMainWindow(QMainWindow):
    ...
    def create_extraction_tab(self):
        ...
        # OCR设置组
        ocr_group = QGroupBox("OCR设置（硬字幕识别）")
        ocr_layout = QGridLayout()
        
        # OCR引擎选择
        ocr_layout.addWidget(QLabel("OCR引擎:"), 0, 0)
        self.ocr_engine_combo = QComboBox()
        engine_options = ["自动选择 (auto)"]
        if IMPROVED_OCR_AVAILABLE:
            try:
                from improved_ocr_extractor import OCR_ENGINES
                if OCR_ENGINES.get('paddleocr', False):
                    engine_options.append("PaddleOCR (推荐)")
                if OCR_ENGINES.get('easyocr', False):
                    engine_options.append("EasyOCR")
                if OCR_ENGINES.get('tesseract', False):
                    engine_options.append("Tesseract")
            except:
                pass
        else:
            engine_options.extend(["EasyOCR (原版)", "Tesseract (原版)"])
        
        self.ocr_engine_combo.addItems(engine_options)
        self.ocr_engine_combo.setToolTip("PaddleOCR在中文识别方面效果最好，推荐使用")
        ...
        self.ocr_interval_spin.setRange(0.1, 10.0)
        ...
        # 置信度阈值
        ocr_layout.addWidget(QLabel("置信度阈值:"), 2, 0)
        ...
        ocr_layout.addWidget(self.ocr_confidence_spin, 2, 1)
        
        # 识别语言
        ocr_layout.addWidget(QLabel("识别语言:"), 3, 0)
        ...
        ocr_layout.addWidget(self.ocr_language_combo, 3, 1)
        
        # GPU加速选项
        ocr_layout.addWidget(QLabel("GPU加速:"), 4, 0)
        self.ocr_gpu_checkbox = QCheckBox("使用GPU加速（如果可用）")
        self.ocr_gpu_checkbox.setChecked(True)
        self.ocr_gpu_checkbox.setToolTip("开启GPU加速可显著提升OCR识别速度\n需要支持CUDA的显卡")
        ocr_layout.addWidget(self.ocr_gpu_checkbox, 4, 1)
        ...
        self.extraction_log.setMaximumHeight(150)
        self.extraction_log.setReadOnly(True)
        result_layout.addWidget(self.extraction_log)
        
        # 字幕文件列表
        self.subtitle_list = QComboBox()
        self.subtitle_list.addItem("未提取字幕")
        result_layout.addWidget(QLabel("选择字幕文件:"))
    ...
    def extract_subtitles(self):
        ...
        self.extraction_thread.progress.connect(self.extraction_log.append)
    ...
    def _get_ocr_settings(self):
        ...
        selected_language = self.ocr_language_combo.currentText()
        languages = language_map.get(selected_language, ['ch_sim', 'en'])
        
        # 解析引擎选择
        engine_map = {
            "自动选择 (auto)": 'auto',
            "PaddleOCR (推荐)": 'paddleocr',
            "EasyOCR": 'easyocr',
            "EasyOCR (原版)": 'easyocr',
            "Tesseract": 'tesseract',
            "Tesseract (原版)": 'tesseract'
        }
        
        selected_engine = self.ocr_engine_combo.currentText()
        engine = engine_map.get(selected_engine, 'auto')
        ...
        return {
            ...
            'engine': engine,
            'use_improved_ocr': IMPROVED_OCR_AVAILABLE and not selected_engine.endswith('(原版)'),
